<template>
  <div class="space-y-6">
    ...
    <div>
      ...
      <p class="text-sm text-gray-500">{{ t('invoices.subtitle') }}</p>
    </div>
    ...
    <div v-else-if="myPayments.length === 0" class="text-center py-16 bg-white rounded-xl border border-gray-100">
      ...
      <p class="font-medium text-gray-600">{{ t('invoices.empty_title') }}</p>
      <p class="text-sm text-gray-400 mb-4">{{ t('invoices.empty_subtitle') }}</p>
      <Button :label="t('invoices.see_pricing')" icon="pi pi-tag" @click="navigateTo('/tarifs')" />
    </div>
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex items-start gap-4">
            ...
            <div>
              ...
              <p class="text-xs text-gray-400 mt-0.5">{{ t('invoices.ref') }} {{ payment.transaction_reference }}</p>
    ...
    <Dialog
      ...
      :header="t('invoices.dialog_title')"
      :modal="true"
    ...
    >
      ...
      <div v-else-if="currentInvoice" class="space-y-4 mt-2">
        ...
        <div class="bg-gradient-primary text-white rounded-xl p-4 text-center">
          ...
          <p class="font-bold text-lg">{{ t('invoices.payment_confirmed') }}</p>
        ...
        <div v-if="currentInvoice.partner_info" class="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800">
          ...
          {{ t('invoices.partner_code') }} <strong>{{ currentInvoice.partner_info.code }}</strong>
          — {{ currentInvoice.partner_info.partner_name }}
        </div>
      </div>

      <template #footer>
        <Button :label="t('invoices.close')" text @click="invoiceDialog = false" />
        <Button
...
const { t } = useI18n()
const { myPayments, loading, fetchMyPayments } = usePayments()
