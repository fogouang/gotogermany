<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <!-- Erreur -->
    <div v-else-if="store.error" class="max-w-lg mx-auto pt-20 px-6 text-center space-y-4">
    ...
    <div v-else-if="subject">
      ...
      <!-- Barre de navigation -->
      <div class="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3">
        ...
        <h1 class="text-sm font-semibold text-gray-800 truncate">{{ subject.title }}</h1>
      ...
      <div class="max-w-3xl mx-auto px-4 pt-6 pb-2">
        <Tabs v-model:value="activeTab">
          <TabList>
            <Tab v-for="(task, i) in subject.tasks" :key="i" :value="i" class="flex items-center gap-2">
              <span :class="['w-5 h-5 rounded-full text-xs font-bold flex items-center justify-center shrink-0 transition-colors',
                wordCount(i) >= task.word_count_min ? 'bg-teal-600 text-white' : 'bg-gray-200 text-gray-500']">
          ...
          <TabPanels>
            <TabPanel v-for="(task, i) in subject.tasks" :key="i" :value="i" class="space-y-4 py-6">

              <!-- Context ad -->
              <div v-if="task.context_ad" class="bg-white border border-gray-200 rounded-xl overflow-hidden">
                <div class="bg-gray-50 border-b border-gray-200 px-5 py-3">
                  <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">{{ t('simulator_subject.ad_context') }}</p>
                </div>
                <div class="px-5 py-4">
              ...
              <div class="bg-amber-50 border border-amber-200 rounded-xl p-5">
                <p class="text-xs font-bold text-amber-700 uppercase tracking-wide mb-2">{{ t('simulator_subject.task_label') }}</p>
              ...
              <div class="flex justify-between pt-2">
                <Button
                  v-if="i > 0"
                  :label="t('simulator_subject.prev_teil')" icon="pi pi-arrow-left"
                  text size="small" @click="activeTab = i - 1"
                />
                <div v-else />
                <Button
                  v-if="i < subject.tasks.length - 1"
                  :label="t('simulator_subject.next_teil')" icon="pi pi-arrow-right" iconPos="right"
                  size="small" severity="secondary" @click="activeTab = i + 1"
                />
              </div>
            </TabPanel>
          </TabPanels>
      ...
      <div class="max-w-3xl mx-auto px-4 pb-10">
        ...
        <div class="flex items-center gap-2 mb-4 flex-wrap">
          <span
            v-for="(task, i) in subject.tasks" :key="i"
            :class="['text-xs px-2 py-1 rounded-full font-medium',
              wordCount(i) >= task.word_count_min ? 'bg-teal-100 text-teal-700' : 'bg-gray-100 text-gray-400']"
          >
        ...
        <div v-if="!store.correction && !store.correcting && !store.correctionError">
          <Button
            :label="t('simulator_subject.correct_btn')" icon="pi pi-sparkles"
            :disabled="!canCorrect" class="w-full" size="large"
        ...
        <!-- Résultat disponible -->
        <div v-else-if="store.correction"
        ...
        >
          <div class="flex items-center justify-between">
            ...
            <Button
              :label="t('simulator_subject.see_results')" icon="pi pi-arrow-right" iconPos="right"
              size="small" @click="goToResults"
            />
          </div>
        </div>
...
const totalWords  = computed(() => answers.value.reduce((sum, _, i) => sum + wordCount(i), 0))
const canCorrect  = computed(() => answers.value.some((_, i) => wordCount(i) > 0))
...
const goToResults = () => router.push(`/dashboard/simulateur/${subjectId}/resultats`)
