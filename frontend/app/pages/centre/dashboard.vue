  ...
  <div>
    ...
    <div v-else-if="!usage?.license" class="text-center py-12">
      ...
      <p class="text-sm text-gray-400 mt-1">
        Contactez ITIA pour activer ou renouveler votre licence.
      </p>
    </div>
    ...
    <div v-else class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <p class="text-xs text-gray-400 mt-1">
            Du {{ formatDate(usage.license.start_date) }} au
            {{ formatDate(usage.license.end_date) }}
          </p>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <p class="text-xs text-gray-400 mt-1">
            avant expiration de la licence
          </p>
        </div>
      </div>
      ...
      >
        ...
        <p class="text-sm text-amber-800">
          Le quota de votre licence est presque atteint ({{
            usage.students_used
          }}/{{ usage.license.max_students }}). Contactez ITIA pour une
          extension si besoin.
        </p>
      </div>
      ...
      >
        ...
        <p class="text-sm text-red-800">
          Votre licence expire dans {{ usage.days_remaining }} jour(s).
          Contactez ITIA pour renouveler.
        </p>
      </div>
        ...
        <div v-else-if="pool" class="space-y-4">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="bg-gray-50 rounded-lg p-4">
              ...
              <p class="text-2xl font-bold text-gray-900">
                {{ pool.default_credits_per_student }}
              </p>
              <p class="text-xs text-gray-400 mt-1">
                crédits à la création d'un compte
              </p>
            </div>
          </div>
          ...
          <div
            v-if="
              pool.ai_credit_pool_balance < pool.default_credits_per_student
            "
          ...
          >
            ...
            <p class="text-xs text-red-700">
              Pool insuffisant pour créer un nouvel étudiant. Contactez ITIA
          ...
          <div class="flex items-end gap-2 pt-2 border-t border-gray-100">
            ...
            <Button
              ...
              :disabled="
                defaultCreditsForm === pool.default_credits_per_student
              "
              @click="handleUpdateDefault"
            />
          </div>
          <p v-if="defaultError" class="text-sm text-red-600">
            {{ defaultError }}
          </p>
        </div>
      </div>
        ...
        <div v-else class="space-y-3">
          ...
          >
            ...
            <span class="text-sm font-semibold text-gray-900"
              >{{ count }} étudiant(s)</span
            >
          </div>
        </div>
      </div>

      <!-- Revenu total (module inscriptions/paiements) -->
      <div class="bg-white rounded-xl border border-gray-200 p-5">
        <h3 class="text-sm font-semibold text-gray-700 mb-4">
          Revenu encaissé - inscriptions & formations
        </h3>

        <div v-if="revenueLoading" class="flex justify-center py-6">
          <i class="pi pi-spin pi-spinner text-2xl text-emerald-600"></i>
        </div>

        <div v-else-if="revenue">
          <div class="bg-emerald-50 rounded-lg p-4 mb-4">
            <p class="text-xs text-emerald-600 mb-1">Revenu total</p>
            <p class="text-2xl font-bold text-emerald-700">
              {{ revenue.total_revenue.toLocaleString() }} FCFA
            </p>
          </div>

          <div
            v-if="revenueBranchEntries.length === 0"
          ...
          <div v-else class="space-y-3">
            <div
              v-for="[branchName, amount] in revenueBranchEntries"
              :key="branchName"
              class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0"
            >
              <span class="text-sm text-gray-700">{{ branchName }}</span>
        ...
        <div v-else-if="revenueError" class="text-sm text-red-600">
          {{ revenueError }}
        </div>
      </div>
      ...
      >
        <div>
          ...
          <p class="text-xs text-gray-400 mt-1">
            Document justificatif à télécharger
          </p>
        </div>
        <Button
          label="Télécharger"
...
const revenue = ref<RevenueSummaryResponse | null>(null);
const revenueError = ref<string | null>(null);
const revenueBranchEntries = computed(() => {
  return Object.entries(revenue.value?.branches_breakdown || {});
});
...
const quotaPercent = computed(() => {
  ...
  return Math.min(
    100,
    Math.round(
      (usage.value.students_used / usage.value.license.max_students) * 100,
    ),
  );
});
...
async function handleUpdateDefault() {
  ...
  const result = await centerStaffStore.updateDefaultCredits(
    defaultCreditsForm.value,
  );
...
async function loadRevenue() {
  ...
  const result = await enrollmentsStore.fetchRevenueSummary();
  if (result.success && result.revenue) {
    revenue.value = result.revenue;
  } else {
    revenueError.value = result.error || "Erreur de chargement du revenu.";
  }
...
onMounted(async () => {
  ...
  if (result.success) {
    ...
    errorMessage.value =
      result.error || "Erreur de chargement de votre licence.";
  }
  loading.value = false;
