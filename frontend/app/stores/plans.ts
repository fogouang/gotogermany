...
import type { PlanResponse, PlanCreate, PlanUpdate } from "#shared/api";
...
export const usePlansStore = defineStore("plans", {
  ...
  getters: {
    ...
    sortedPlans: (state) =>
      [...state.plans].sort((a, b) => a.display_order - b.display_order),
  },
  actions: {
    _ensureApiConfig() {
    ...
    async fetchPlans() {
      ...
      try {
        ...
        this.error =
          error.body?.detail || "Erreur lors du chargement des plans";
    ...
    async updatePlan(planId: string, data: PlanUpdate) {
      this._ensureApiConfig();
      this.loading = true;
      this.error = null;
      try {
        const updated = await PlansService.updatePlanApiV1PlansPlanIdPatch(
          planId,
          data,
        );
    ...
    async deletePlan(planId: string) {
      ...
      try {
        ...
        return {
          success: false,
          error: error.body?.detail || "Erreur lors de la suppression",
        };
      }
    },
  },
