    ...
    <div v-if="teil.format_type === 'oral_interaction'" class="space-y-5">
      ...
      >
        ...
        <ul class="space-y-2">
          ...
          >
            <span
              class="w-5 h-5 bg-teal-100 text-teal-700 rounded-full flex items-center justify-center text-xs font-bold shrink-0"
            >
              {{ (i as number) + 1 }}
            </span>
            {{ prompt }}
      ...
      <div v-else class="space-y-5">
        ...
        <div class="bg-white border-2 border-teal-200 rounded-xl p-6 min-h-24">
          <div class="flex items-start gap-3">
            <span
              class="w-8 h-8 bg-teal-600 text-white rounded-full flex items-center justify-center text-sm font-bold shrink-0"
            >
              {{ activeSlide + 1 }}
            </span>
    ...
    <div v-else-if="teil.format_type === 'oral_feedback'" class="space-y-5">
      ...
      <div>
        ...
        <AudioRecorder
          ...
          :existing-audio-url="
            answers[question?.id]?.user_answer?.audio_url || ''
          "
          @recorded="onRecorded"
        />
      </div>
...
const onRecorded = () => {
  // Optionnel : marquer la question comme "complétée" localement
  if (!question.value) return;
  const existing = props.answers[question.value.id]?.user_answer || {};
